...
void TimerMan::DeleteEvent(TimerEvent::Name pEventName)
{
	TimerEvent* pEvent = Instance().pHead;
	TimerEvent* pTmp = nullptr;
	while (pEvent != nullptr)
	{
		if (pEvent->GetName() == pEventName)
		{
			pTmp = pEvent;
			pEvent = (TimerEvent*)pEvent->next;
			RemoveEvent(pTmp);
			delete pTmp;
			break;
		}
...
const float TimerMan::GetCurrTime()
{
