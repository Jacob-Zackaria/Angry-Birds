...
GameObjectName::Name SlingQueue::GetRandomName()
{
	uint32_t newRand = (rand() % 6);
	switch ((BirdNames)newRand)
	{
	case BirdNames::RED_BIRD:
		return GameObjectName::Name::RedBird0;
		break;
	case BirdNames::BLUE_BIRD:
		return GameObjectName::Name::BlueBird0;
		break;
	case BirdNames::YELLOW_BIRD:
		return GameObjectName::Name::YellowBird0;
		break;
	case BirdNames::BOMB:
		return GameObjectName::Name::BombBird0;
		break;
	case BirdNames::WHITE_BIRD:
		return GameObjectName::Name::WhiteBird0;
		break;
	case BirdNames::BIG_BIRD:
		return GameObjectName::Name::BigBird0;
		break;
	default :
		return GameObjectName::Name::NOT_INITIALIZED;
		break;
	}
}
...
{
	srand((unsigned int)TimerMan::GetCurrTime());
	...
	for (uint8_t i = SlingQueue::GetCurrentCount(); i < SlingQueue::GetBirdLimit(); i++)
	{
		...
		tC = BirdFactory::CreateBird(pMan->GetRandomName(), rect, Rect(), true);
		SceneManager::AddSceneObject(SceneManager::GetCurrentScene(), tC);
...
void SlingQueue::ReloadBirds(float newBaseX, float newBaseY)
{
	...
	for (uint8_t i = SlingQueue::GetCurrentCount(); i < SlingQueue::GetBirdLimit(); i++)
	{
		...
		GameObject2D* tC = BirdFactory::CreateBird(SlingQueue::privGetInstance()->GetRandomName(), rect, Rect(), true);
		SceneManager::AddSceneObject(SceneManager::GetCurrentScene(), tC);
		SlingQueue::AddBird(tC);
	}
}
