...
Tower4::Tower4(const float newX, const float newY, const int height)
	:
...
void Tower4::Build()
{
	...
	for (int i = 0; i < towHeight; i++)
	{
		...
		{
			/*Rect newRect = Rect(baseX - (0.5f * stickWidth) - 45.0f, baseY + (0.5f * stickWidth) + (i * stickWidth) + 20.0f, stickWidth, stickHeight);
			GameObject2D* pObj = ObjectFactory::CreateObject(GameObjectName::Name::GlassPlatform0, newRect, Rect(), true);
			SceneManager::AddSceneObject(SceneManager::GetCurrentScene(), pObj);
			pObj->GetBody()->SetType(b2BodyType::b2_dynamicBody);
			pObj->SetOrigAngle(MATH_PI2);*/

			Rect newRect = Rect(baseX - (0.5f * stickWidth), baseY + (0.5f * stickWidth) + (i * stickWidth) + 80.0f, stickWidth - 10.0f, stickHeight);
			GameObject2D* pObj = ObjectFactory::CreateObject(GameObjectName::Name::GlassPlatform0, newRect, Rect(), true);
			SceneManager::AddSceneObject(SceneManager::GetCurrentScene(), pObj);
			pObj->GetBody()->SetType(b2BodyType::b2_dynamicBody);
			pObj->SetOrigAngle(MATH_PI2);
			...
			// MinionPig0
			newRect = Rect(baseX + 5.0f, baseY + (i * stickWidth) + stickHeight + 85.0f, 35.0f, 35.0f);
			pObj = PigFactory::CreatePig(GameObjectName::Name::MinionPig0, newRect, Rect(), true);
			SceneManager::AddSceneObject(SceneManager::GetCurrentScene(), pObj);
			b2Fixture* editFixture = pObj->GetBody()->GetFixtureList();
			editFixture->SetFriction(0.3f);
			...
			newRect = Rect(baseX, baseY + (i * stickWidth) + stickWidth + 85.0f, stickWidth + 15.0f - 10.0f, stickHeight);
			pObj = ObjectFactory::CreateObject(GameObjectName::Name::WoodPlatform0, newRect, Rect(), true);
			SceneManager::AddSceneObject(SceneManager::GetCurrentScene(), pObj);
			pObj->GetBody()->SetType(b2BodyType::b2_dynamicBody);

			newRect = Rect(baseX + (0.5f * stickWidth), baseY + (0.5f * stickWidth) + (i * stickWidth) + 80.0f, stickWidth - 10.0f, stickHeight);
			pObj = ObjectFactory::CreateObject(GameObjectName::Name::GlassPlatform0, newRect, Rect(), true);
			SceneManager::AddSceneObject(SceneManager::GetCurrentScene(), pObj);
			pObj->GetBody()->SetType(b2BodyType::b2_dynamicBody);
			pObj->SetOrigAngle(MATH_PI2);
